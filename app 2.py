    ...
    if st.button('Prever Obesidade', key='predict_button'):
        if model is not None:
            ...
            features_to_visualize = {
                'vegetais': {'label': 'Consumo de Vegetais', 'positive': True}, # Higher is generally positive
                'ref_principais': {'label': 'Refeições Principais/Dia', 'positive': True}, # Within a healthy range is positive
                'agua': {'label': 'Consumo de Água (Litros)', 'positive': True}, # Higher is generally positive
                'atv_fisica': {'label': 'Atividade Física', 'positive': True}, # Higher is generally positive
                'atv_eletronica': {'label': 'Tempo de Tela', 'positive': False}, # Higher is generally negative
                'historico': {'label': 'Histórico Familiar', 'positive': False, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'al_calorico': {'label': 'Alimentos Calóricos', 'positive': False, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'ctrl_caloria': {'label': 'Controle de Calorias', 'positive': True, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'entre_ref': {'label': 'Comer Entre Refeições', 'positive': False, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'fumante': {'label': 'Fumante', 'positive': False, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'alcool': {'label': 'Consumo de Álcool', 'positive': False, 'binary_labels': {0: 'Não', 1: 'Sim'}},
                'transporte': {'label': 'Transporte Ativo (Caminhar/Bike)', 'positive': True, 'binary_labels': {0: 'Sim', 1: 'Não'}}, # 0 is positive here
            }
            ...
            for feature_name, info in features_to_visualize.items():
                if feature_name in dados_entrada:
                    ...
                    # For binary features, use the label corresponding to the value
                    if binary_labels is not None:
                         display_value = binary_labels.get(value, str(value)) # Use label if available, otherwise raw value
                         # Determine color based on the value and positive/negative status
                    ...
                    # For numerical features, use the raw value and determine color based on a simple threshold or just use a single color
                    else:
                         display_value = value
                         # For simplicity, let's color numerical features based on whether higher is positive or negative
                         # A more sophisticated approach would involve comparing to a healthy range or mean/median
                         color = 'blue' if is_positive else 'red'


                    chart_data.append({'Feature': label, 'Value': display_value, 'Color': color})
            ...
            bars = ax.bar(chart_df['Feature'], chart_df['Value'], color=chart_df['Color'])

            # Add labels and title
            ax.set_ylabel('Valor/Resposta do Usuário')
