...
def main_page(model, mean_cv_accuracy, std_cv_accuracy, test_accuracy):
    ...
    perguntas_amigaveis_widgets = {
        ...
        'transporte': {"pergunta": "Seu meio de transporte principal envolve caminhada ou bicicleta? ", "tipo": "radio", "opcoes": {0: 'Sim', 1: 'Não'}}, # Corrected: Removed extra single quote after opcoes
            }
    ...
    if st.button('Prever Obesidade', key='predict_button'):
        if model is not None:
            ...
            x_labels = []
            bar_colors = []
            bar_values = [] # Store numerical values for plotting
            ...
            for feature_name, info in features_to_visualize.items():
                if feature_name in dados_entrada:
                    ...
                    if feature_type == 'binary':
                        # Use the numerical value (0 or 1) for plotting height
                        ...
                        bar_colors.append(color)
                         ...
                         # Use the raw numerical value for plotting height
                         bar_values.append(value)
            ...
            bars = ax.bar(x_labels, bar_values, color=bar_colors) # Use bar_values for height
            ...
            # Add text labels on top of the bars (use original values or binary labels)
            for bar, feature_name in zip(bars, features_to_visualize.keys()):
                 ...
                 if feature_type == 'binary' and binary_labels is not None:
                      # For binary features, display the text label (Sim/Não)
